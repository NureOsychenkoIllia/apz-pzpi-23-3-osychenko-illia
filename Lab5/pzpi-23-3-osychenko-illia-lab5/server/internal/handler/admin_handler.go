...
type AdminHandler struct {
	...
	dataMigrationService service.DataMigrationService
}
...
//	@Description	Створює зашифровану резервну копію бази даних (pg_dump + AES-256-GCM)
//	@Tags			Admin
//	@Accept			json
//	@Produce		json
...
//	@Description	Повертає метадані резервних копій (без вмісту)
//	@Tags			Admin
//	@Accept			json
//	@Produce		json
...
//	@Description	Розшифровує та відновлює базу даних з вказаної резервної копії
//	@Tags			Admin
//	@Accept			json
//	@Produce		json
...
// ExportData експортує операційні дані з GDPR-анонімізацією
//
//	@Summary		Експортувати операційні дані
//	@Description	Повертає маршрути, автобуси, рейси та анонімізованих користувачів для міграції між середовищами
//	@Tags			Admin
//	@Produce		json
//	@Success		200	{object}	service.DataExport
//	@Failure		500	{object}	ErrorResponse
//	@Security		BearerAuth
//	@Router			/admin/data/export [get]
func (h *AdminHandler) ExportData(c *fiber.Ctx) error {
	export, err := h.dataMigrationService.ExportData(c.Context())
	if err != nil {
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
	c.Set("Content-Disposition", "attachment; filename=busoptima_data_export.json")
	return c.JSON(export)
}

// ImportData імпортує операційні дані з анонімізованого пакету міграції
//
//	@Summary		Імпортувати операційні дані
//	@Description	Відновлює маршрути та автобуси з раніше експортованого пакету. Персональні дані не імпортуються.
//	@Tags			Admin
//	@Accept			json
//	@Produce		json
//	@Param			data	body		service.DataExport	true	"Анонімізований пакет міграції"
//	@Success		200		{object}	map[string]interface{}
//	@Failure		400		{object}	ErrorResponse
//	@Failure		500		{object}	ErrorResponse
//	@Security		BearerAuth
//	@Router			/admin/data/import [post]
func (h *AdminHandler) ImportData(c *fiber.Ctx) error {
	var export service.DataExport
	if err := c.BodyParser(&export); err != nil {
		return c.Status(400).JSON(fiber.Map{"error": "Invalid request body: " + err.Error()})
	}
	if err := h.dataMigrationService.ImportData(c.Context(), &export); err != nil {
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
	return c.JSON(fiber.Map{
		"message":      "Data imported successfully",
		"routes_count": len(export.Routes),
		"buses_count":  len(export.Buses),
	})
}
