...
SELECT 
    ...
    CASE 
        WHEN t.route_id = 1 THEN 0.80 + (random() * 0.18)
        WHEN t.route_id = 2 THEN 0.55 + (random() * 0.33)
        ELSE 0.54 + (random() * 0.38)
...
(3, 'UPDATE', 'trips', (SELECT id FROM trips WHERE status = 'in_progress' AND current_passengers = 28 LIMIT 1), '{"current_passengers": 25}', '{"current_passengers": 28}', '10.0.2.16', NOW() - INTERVAL '20 minutes'),
...
(3, 'CREATE', 'passenger_events', (SELECT MAX(id)-1 FROM passenger_events), '{}', '{"trip_id": 2, "event_type": "entry"}', '10.0.2.16', NOW() - INTERVAL '10 minutes');
