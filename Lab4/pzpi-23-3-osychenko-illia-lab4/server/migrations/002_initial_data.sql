...
INSERT INTO permissions (name, description) VALUES
    ...
    ('trips:write', 'Створення та редагування рейсів'),
...
INSERT INTO roles (name, description) VALUES
    ...
    ('driver', 'Водій - мобільний клієнт та поточний рейс'),
...
WHERE r.name = 'dispatcher' AND p.name IN 
    ('routes:read', 'trips:write', 'buses:read', 'analytics:read', 'reports:export');

-- Водій
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permissions p
WHERE r.name = 'driver' AND p.name IN
    ('routes:read', 'trips:write', 'buses:read');

-- Бізнес-адміністратор
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permissions p
WHERE r.name = 'admin' AND p.name IN 
    ('routes:read', 'routes:write', 'trips:write', 'buses:read', 'buses:write', 
...
INSERT INTO users (email, password_hash, full_name, role_id) VALUES
    ('dispatcher@busoptima.ua', '$2a$10$o2R8cGeNWdPIL9ZinZ7CU.cj5kRweixh19TkHFfOvh9uE40v9H/5y', 'Іван Петренко', 
     (SELECT id FROM roles WHERE name = 'dispatcher')),
    ('driver@busoptima.ua', '$2a$10$o2R8cGeNWdPIL9ZinZ7CU.cj5kRweixh19TkHFfOvh9uE40v9H/5y', 'Петро Коваленко',
     (SELECT id FROM roles WHERE name = 'driver')),
    ('admin@busoptima.ua', '$2a$10$o2R8cGeNWdPIL9ZinZ7CU.cj5kRweixh19TkHFfOvh9uE40v9H/5y', 'Марія Іваненко', 
     (SELECT id FROM roles WHERE name = 'admin')),
    ('tech@busoptima.ua', '$2a$10$o2R8cGeNWdPIL9ZinZ7CU.cj5kRweixh19TkHFfOvh9uE40v9H/5y', 'Олексій Сидоренко', 
...
INSERT INTO trip_analytics (trip_id, total_passengers, max_passengers, avg_occupancy_rate, revenue, fuel_cost, driver_cost, other_costs, profit, profitability_percent) VALUES
    (1, 35, 50, 0.70, 5250.00, 1200.25, 800.00, 150.00, 3099.75, 144.18),
    (3, 32, 45, 0.71, 5760.00, 1087.50, 900.00, 120.00, 3652.50, 173.34);
